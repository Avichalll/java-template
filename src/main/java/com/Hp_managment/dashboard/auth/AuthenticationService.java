package com.Hp_managment.dashboard.auth;
...
import com.Hp_managment.dashboard.Role.Role;
import com.Hp_managment.dashboard.Role.RoleRepository;
import com.Hp_managment.dashboard.Role.URole;
import com.Hp_managment.dashboard.User.JwtService;
import com.Hp_managment.dashboard.User.Token;
import com.Hp_managment.dashboard.User.TokenRepository;
import com.Hp_managment.dashboard.User.User;
import com.Hp_managment.dashboard.User.Userrepository;
import com.Hp_managment.dashboard.handler.AccountNotVerfiedException;
import com.Hp_managment.dashboard.handler.DuplicateEmailException;
import com.Hp_managment.dashboard.handler.PasswordNotMatchException;
import com.Hp_managment.dashboard.handler.ResourceNotFoundException;
...
public class AuthenticationService {
    ...
    public void sendSms(String otp, String email) throws IOException {
        ...
        String routeId = "1";
        String message = String.format("Please use the code %s to log in on the Nodex App. Please do not share this code with anyone for security reasons. For more detail visit https://medantrik.com/ Team Medantrik", otp);
    ...
    public AuthenticationResponse authenticate(@Valid AuthenticaionRequest request) throws MessagingException,IOException{

        User user1 = userrepository.findByContactNumber(request.getContactNumber())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid PhoneNumber or password"));

                if (!user1.isEnabled()) {
                    sendValidationEmail(user1);
                    throw new AccountNotVerfiedException("Account not verified. Please check your email for the verification code.");
                }  
    ...
    public String resendOtp(ResendOtpRequest resendOtpRequest) throws MessagingException,IOException {
        User user = userrepository.findByContactNumber(resendOtpRequest.getContactNumber())
                .orElseThrow(() -> new ResourceNotFoundException(resendOtpRequest.getContactNumber() +" is Not Registered"));
    ...
    public void changeAuthenticationPassword(ChangePasswordRequest changePasssword, Authentication connnectedUser) throws BadRequestException {
    ...
    public String forgetPassword(ForgetPasswordRequest forgetPassword)throws MessagingException,IOException {
        User user = userrepository.findByContactNumber(forgetPassword.getContactNumber()).orElseThrow(()-> new ResourceNotFoundException("Not Registered"));
        String otp = generateAndSaveActivationToken(user);
    ...
    public String resetForgottendPassword(ResetPasswordRequest resetPasswordRequest) {
        ...
        User user =userrepository.findByContactNumber(resetPasswordRequest.getContactNumber()).orElseThrow(()-> new ResourceNotFoundException("user not Found"));
    ...
    public String otpVerification(OtpVerficationRequest otpVerficationRequest) {
        ...
        Token savedToken = tokenRepository.findByToken(otpVerficationRequest.getOtp()).orElseThrow(()-> new ResourceNotFoundException("Invalid Otp"));
